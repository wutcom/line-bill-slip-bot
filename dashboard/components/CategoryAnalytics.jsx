...
export default function CategoryAnalytics({ data }) {
  const max = Math.max(...data.categories.map((row) => row.totalAmount), 1);
  const total = data.categories.reduce((sum, row) => sum + row.totalAmount, 0);
    ...
    <div className="category-layout">
      ...
      <section className="section-block">
        ...
        <div className="trend-list">
          ...
          {data.monthlyTrend.slice(0, 24).map((row, index) => (
